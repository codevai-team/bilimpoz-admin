...
export default function EditLessonPage() {
  ...
  const [_group, setGroup] = useState<LessonGroup | null>(null);
    ...
    try {
      ...
      if (courseResponse.ok) {
        ...
        if (foundGroup) {
          ...
          // Находим урок в группе
          const foundLesson = foundGroup.lessons?.find((l: Lesson) => l.id === lessonId);
          if (foundLesson) {
            ...
            setFormData({
              ...
              lecture_url: foundLesson.lecture_url,
              type: foundLesson.type,
              access_type: foundLesson.access_type,
              total_questions: foundLesson.total_questions,
              random_questions: foundLesson.random_questions,
              mistake_chance: foundLesson.mistake_chance,
              lesson_points: foundLesson.lesson_points
            });
            ...
            setBreadcrumbs([
              ...
              { id: foundLesson.id, title: foundLesson.title, type: 'lesson' },
              { id: 'edit', title: 'Редактировать', type: 'edit' }
            ]);
          }
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiRequest(`/api/courses/${courseId}/groups/${groupId}/lessons/${lessonId}`, {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        setSaveMessage('Урок успешно сохранен!');
        setTimeout(() => setSaveMessage(''), 3000); // Убираем сообщение через 3 секунды
      } else {
        const errorData = await response.json();
        console.error('Ошибка обновления урока:', errorData);
      }
    } catch (error) {
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[400px]">
          <Icons.AlertCircle className="h-12 w-12 text-red-400 mb-4" />
          <h2 className="text-xl font-semibold text-white mb-2">Урок не найден</h2>
          <p className="text-gray-400 mb-4">Урок с указанным ID не существует</p>
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white">Редактировать урок</h1>
            {saveMessage && (
